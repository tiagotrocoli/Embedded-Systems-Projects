...
EventGroupHandle_t tempCriticalSection;
...
int main(void)
{
  ...
  tempCriticalSection = xEventGroupCreate();
	// Set the bit to initialize one of the threads
	xEventGroupSetBits(tempCriticalSection, BIT_1);
  ...
  int status1 = xTaskCreate(showTemperature2sec, "Show_Temperature_for_2sec", 2*configMINIMAL_STACK_SIZE, NULL, 5, &task1);
	int status2 = xTaskCreate(uartController, "UART_Controller", 2*configMINIMAL_STACK_SIZE, NULL, 5, &task3);
	int status3 = xTaskCreate(showTemperature5sec, "Show_Temperature_for_5sec", 2*configMINIMAL_STACK_SIZE, NULL, 5, &task2);
...
void showTemperature2sec(void *param){
	...
	while(1){
		...
		xEventGroupWaitBits(tempCriticalSection, BIT_1, pdFALSE, pdFALSE ,portMAX_DELAY);
		init_ADC1();
		temperature = getValueFromADC1();
		// after leaving the critical section, it allows another task to access it
		xEventGroupClearBits(tempCriticalSection, BIT_1);
		xEventGroupSetBits(tempCriticalSection, BIT_2);
...
void showTemperature5sec(void *param){
	...
	while(1){
		...
		// Provide the CPU temperature that is a critical section
		xEventGroupWaitBits(tempCriticalSection, BIT_2, pdFALSE, pdFALSE ,portMAX_DELAY);
		temperature = getValueFromADC1();
		// after leaving the critical section, it allows another task to access it
		xEventGroupClearBits(tempCriticalSection, BIT_2);
		xEventGroupSetBits(tempCriticalSection, BIT_1);
